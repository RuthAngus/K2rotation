...
def delta_nu(fs, s2n, eid, dnu, sub=1, smooth=False):
    ...
    plt.subplot(3, 1, 2)
#     plt.imshow(np.exp(acor), cmap="gray_r", interpolation="nearest",
#                aspect="auto", vmin=min(np.exp(acor)), vmax=max(np.exp(acor)))
