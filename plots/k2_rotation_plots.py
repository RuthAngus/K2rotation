...
def read_data(epid):
    # read the data
    data = fitsio.read("../data/ktwo%s-c01_lpd-lc.fits" % epid)
    aps = fitsio.read("../data/ktwo%s-c01_lpd-lc.fits" % epid, 2)
...
#     plt.ylabel("$\mathrm{S/N}$")
    ...
    plt.legend()
    plt.savefig("K2_rotation_%s" % epid)
...
if __name__ == "__main__":
    ...
    amp2s, s2n, w  = K2pgram(x, y, fs, AT, ATA)
