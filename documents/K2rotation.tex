...
in {\it K2} data, {\it Kepler's} 2-wheeled extention, is relevent to a long
...
any detrending.
The SIP method is also suited to measuring stellar rotation periods, despite
the fact that long-term systematics exist on similar timescales.
The SIP code is available at https://github.com/RuthAngus/SIPK2.
...
As well as providing data that has lead to the discovery thousands of
...
https://github.com/astroML/gatspy/tree/master/gatspy/periodic} (LS) periodogram
of the raw, simple aperture photometry\footnote{The method used to extract this photometry is described in \textsection \ref{sec:Method}} of EPIC 201183188,
a pulsating giant star.
...
The brightness fluctuations clearly visible in the light curve of this
target are produced by active regions rotating with the stellar surface,
reducing the stellar flux periodically.
...
Each of these 3 methods measures a rotation period of around 20 days for this
target.
...
Whereas the ACF method is able to measure a rotation of $\sim$ 6 days, the
LS periodogram of the detrended light curve shows high power at long
periods and although there is a small peak at the rotation period of the
star, it is not the dominant periodic signal.
The SIP method is, by definition, insensitive to these long-term systematics
and is able to measure a period of $\sim$ 7 days.
...
for measuring stellar rotation periods, ideally we would inject
sinusoids into raw {\it K2} light curves, detrend them using
the VJ15 method (for example) and attempt to recover signals using a LS
...
We have demonstrated that the SIP method is capable of detecting periodicities
in {\it K2} data in the region of frequency space relevent to the study of
asteroseismic oscillations in giant stars and for any signals with a timescale
close to 6 hours.
The SIP is also effective at measuring stellar rotation periods and is an
...
It is a pleasure to thank Dan Huber (Sydney) who provided comments for this
paper and useful asteroseismology tips.
We would also like to thank Suzanne Aigrain (Oxford) David Hogg (NYU) for
their comments and Andrew Vanderburg (Harvard) and Ben Montet (Harvard)
for their extremely helpful suggestions and recommendations regarding this
project.
...
period of 20 days for this star. {\it Bottom}: The LS periodogram
...
\caption{{\it Top}: Raw light curve of EPIC \rpc. {\it Bottom}: An
SIP, produced by modelling the data using the top 150 ELCs
plus a sine and cosine function at a range of frequencies. The highest peak in
the SIP is centred at 25 days.}
...
\begin{figure*}
\begin{center}
\includegraphics[width=6in, clip=true]{rotation201142127.pdf}
\caption{{\it Top}: Light curve of EPIC \rpc, detrended using the method
of VJ14. {\it Middle}: Autocorrelation function of the
detrended light curve. The autocorrelation function method measures a rotation
period of 6 days for this star. {\it Bottom}: The LS periodogram
of the detrended light curve. The highest peak in the periodogram is centred at
24 days.}
...
\begin{figure*}
\begin{center}
\includegraphics[width=6in, clip=true]{K2_rotation_201142127.pdf}
\caption{{\it Top}: Raw light curve of EPIC \rpcc. {\it Bottom}: An
SIP, produced by modelling the data using the top 150 ELCs
plus a sine and cosine function at a range of frequencies, measuring
a rotation period of 7 days.}
...
\begin{figure*}
\begin{center}
\includegraphics[width=6in, clip=true]{top5pgram_201133037.pdf}
\caption{The 5 highest-weighted ELCs for EPIC \rpc, with their corresponding LS
	periodograms.
	The 3 highest-weighted ELCs have power at long periods.
	The SIP is insensitve to these long-term systematics, but they may
	still be present in detrended data.}
\label{fig:top5}
\end{center}
\end{figure*}
